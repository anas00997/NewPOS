...
class OrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($orders)
                ...
                ->rawColumns(['saleId', 'customer', 'item', 'sub_total', 'total', 'status', 'action'])
                ->toJson();
        }
    ...
    public function store(Request $request)
    {
        ...
        $carts = PosCart::with('product')
            ...
            ->where('branch_id', auth()->user()->branch_id)
            ->get();
        $order = null; // Initialize $order outside the transaction
        DB::transaction(function () use ($request, $carts, &$order) {
            $order = Order::create([
                'customer_id' => $request->customer_id,
                'user_id' => $request->user()->id,
                'branch_id' => $request->user()->branch_id,
                // Business day cutoff logic for created_at
                'created_at' => (function() {
                    $cutoffConfig = config('business_day.cutoff_time');
                    $cutoffTime = \Carbon\Carbon::today()->setTimeFromTimeString($cutoffConfig);
                    $currentTime = \Carbon\Carbon::now();
                    return $currentTime->gt($cutoffTime)
                        ? $currentTime->copy()->addDay()->format('Y-m-d H:i:s')
            ...
            foreach ($carts as $cart) {
                $mainTotal = $cart->price * $cart->quantity;
                $totalAmountOrder += $mainTotal;
